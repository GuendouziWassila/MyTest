...
class AccountGUI:
    ...
    def verify_new_user(acct):
        ...
        try:
            ...
            for userTemp in userDataFile:
                print('In verify_new_user and userTemp from file is: ' + userTemp)
            ...
            if (valid == False):   # if it exists in the file
                tk.messagebox.showinfo('Invalid User Name','That user name already exists.')
    ...
    def verify_new_pass(acct, user):
        ...
        if acct.verify_pass(txt):   # if the call to verify password criteria is met
            ...
            passwordFile = open('acct_user_passwords.txt', 'a')   # open the user file to append it
